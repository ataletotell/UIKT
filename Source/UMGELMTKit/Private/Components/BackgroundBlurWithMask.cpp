...
TSharedRef<SWidget> UBackgroundBlurWithMask::RebuildWidget()
{
	...
	{
		CastChecked<UBackgroundBlurWithMaskSlot>(GetContentSlot())->BuildSlot(MyBackgroundBlur.ToSharedRef());
	}
...
bool UBackgroundBlurWithMask::IsTickable() const
{
	return (HasAnyFlags(RF_ClassDefaultObject) == false)
		&& (bUseMaskMaterial || bBlurTransitionActive);
}
...
void UBackgroundBlurWithMask::Tick(float DeltaTime)
{
	...
	if (bBlurTransitionActive)
	{
		const float NewStrength = FMath::FInterpConstantTo(BlurStrength, BlurStrengthTarget, DeltaTime, BlurTransitionRate);
		SetBlurStrength(NewStrength);
		if (FMath::IsNearlyEqual(NewStrength, BlurStrengthTarget, 0.01f))
		{
			SetBlurStrength(BlurStrengthTarget);
			bBlurTransitionActive = false;
		}
	}
}

void UBackgroundBlurWithMask::SetBlurStrengthSmooth(float InTargetStrength, float InRate)
{
	BlurStrengthTarget    = FMath::Max(0.f, InTargetStrength);
	BlurTransitionRate    = FMath::Max(0.1f, InRate);
	bBlurTransitionActive = !FMath::IsNearlyEqual(BlurStrength, BlurStrengthTarget, 0.01f);
	if (!bBlurTransitionActive)
	{
		SetBlurStrength(BlurStrengthTarget);
	}
...
void UBackgroundBlurWithMask::RedrawMaskMaterial()
{
	if (!MaskMaterialRenderTarget)
	{
		...
		MaskMaterialRenderTarget->TargetGamma = 0.0f;
