...
FElementGIFDecoder::~FElementGIFDecoder()
{
	Close();
}
...
bool FElementGIFDecoder::LoadFromMemory(const uint8* InBuffer, uint32 InBufferSize)
{
	int gifError = 0;
	...
	if (mGIF == nullptr)
	{
		...
		UE_LOG(LogElementAnimTexture, Error, TEXT("FElementGIFDecoder: GIF file open failed, %s."), *Error);
		return false;
	}
	...
	{
		FString Error(GifErrorString(gifError));
		UE_LOG(LogElementAnimTexture, Error, TEXT("FElementGIFDecoder: GIF file load failed, %s."), *Error);
		return false;
	}
...
uint32 FElementGIFDecoder::NextFrame(uint32 DefaultFrameDelay, bool bLooping)
{
...
void FElementGIFDecoder::SeekToFrame(uint32 FrameIndex)
{
	if (!mGIF) return;

	// Clamp FrameIndex
	if (FrameIndex >= (uint32)mGIF->ImageCount)
	{
		FrameIndex = 0;
	}
...
void FElementGIFDecoder::Reset()
{
	mCurrentFrame = 0;
	mLoopCount = 0;
	mDoNotDispose = false;
	
	// Reset the frame buffer to background (important for non-disposed frames)
	if (mGIF)
	{
		int transparentColor = -1;
		// Determine transparent color from first frame gcb if possible, or just assume standard reset
		// For simplicity, just clearing:
		ClearFrameBuffer(mGIF->SColorMap, true);
	}
}

uint32 FElementGIFDecoder::GetWidth() const
{
	if (mGIF)
		return mGIF->SWidth;
	else
...
uint32 FElementGIFDecoder::GetHeight() const
{
	if (mGIF)
		return mGIF->SHeight;
	else
...
const FColor* FElementGIFDecoder::GetFrameBuffer() const
{
	return mFrameBuffer.GetData();
}

uint32 FElementGIFDecoder::GetDuration(uint32 DefaultFrameDelay) const
{
	if (!mGIF)
		return 0;
...
bool FElementGIFDecoder::SupportsTransparency() const
{
	if (!mGIF)
		return false;
...
void FElementGIFDecoder::ClearFrameBuffer(ColorMapObject* ColorMap,
	bool bTransparent) 
{
	FColor bg = { 0, 0, 0, 255 };
...
void FElementGIFDecoder::GCB_Background(int left, int top, int width, int height,
	ColorMapObject* colorMap,
	bool bTransparent)
{
